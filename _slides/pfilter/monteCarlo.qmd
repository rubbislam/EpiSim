...
Let's examine some *Monte Carlo* approaches for evaluating this and other difficult integrals.
...
\end{equation*}
allows us to generalize the Monte Carlo integration theorem to give the **Monte Carlo importance sampling theorem**:
\begin{equation*}
  \expect{h(X)} \approx \frac{1}{J}\,\sum_{j=1}^{J} h(Y_j)\,\frac{f(Y_j)}{g(Y_j)}.
\end{equation*}

If we call $w_j=f(Y_j)/g(Y_j)$ the *importance weights*, we can write
\begin{equation*}
  \expect{h(X)} \approx \frac{1}{J}\,\sum_{j=1}^{J} w_j\,h(Y_j).
\end{equation*}

Since $\expect{w_j} = \expect{f(Y)/g(Y)}=1$, we can modify this formula to give a *self-normalized importance sampling* estimate,
\begin{equation*}
...
Suppose a random variable $X$ is *uniformly distributed* over a region $D\subset\mathbb{R}^{d}$.
...
@Robert2004 refer to this as the *fundamental theorem of simulation*.
...
1.  Draw $Y\!\sim\!g$ and $U\!\sim\!\dist{Uniform}{0,M}$.
...
**The rejection method for simulating an arbitrary random variable.**

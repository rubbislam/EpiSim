---
title: "Meta-Population extensions"
author:
  - Spencer J. Fox
  - Qianying (Ruby) Lin
  - Jesse Wheeler
...
We don't even need to change the code!

```{r ppompMif2, eval=TRUE}
mif2Out <- mif2(
  measSIR, rw.sd = rw_sd(rho = 0.02, R_0 = 0.02), 
  Nmif = 10, Np = 200, cooling.fraction.50 = 0.5, 
  block = TRUE  # block = TRUE does MPIF, will become default.
  )
```

Capable of fitting more than 2500 time series simultaneously [@wheeler25].
...
## Model specification (rprocess)

```{r eval=FALSE}
measles_rprocess <- spatPomp_Csnippet(
  unit_statenames = c('S','E','I','C'),
  code = "double beta, seas, Ifrac, mu[7], dN[7];
  int u, v;

...
Note that, the macOS and Linux systems automatically export the global environment to each core, 
while Windows does not do that.
The setup to let `foreach` to use `doParallel` backend to run parallel computing will be a bit different.
...
The following codes (also see the `/scripts/exercise_parallel_*.R` script) is an example of setting up a parallel computing scheme.

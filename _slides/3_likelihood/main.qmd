...
library(doFuture)             # load doFuture/future
plan(multisession)            # using multiple R sessions
                              # handling global environment
```

The second line tells `foreach` that we will use the `doFuture` backend, along with `%dofuture%` when we set up the `foreach` loop.
The third line explicitly exports the global environment to each of the session on Windows machines; macOS/Linux does that automatically.

<!-- ## Parallel computing: Windows -->

## Parallel computing: Exercise

The following codes (or the `/scripts/exercise_parallel.R` script) is an example of setting up a parallel computing scheme.

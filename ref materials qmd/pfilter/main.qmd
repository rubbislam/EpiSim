...
\begin{center}
    \includegraphics[height=4cm]{../graphics/lec3ll.png}
  \end{center}
...
## Monte Carlo likelihood by direct simulation {.allowframebreaks}
  ...
  -  Although **this section is a demonstration of what not to do**, it serves as an introduction to the general approach of [Monte Carlo integration](monteCarlo.html\#the-fundamental-theorem-of-monte-carlo-integration).
...
\begin{equation} 
  ...
  -  We can see this happening in practice for the measles outbreak data: [supplementary material](directSimulation.html).
...
\begin{equation*}
  ...
  -  References on the particle filter include @Kitagawa1987, @Arulampalam2002, @Doucet2001, @King2016.
...
\begin{center}
    \includegraphics[height=4cm]{../graphics/lec3pf.png}
  \end{center}


## Parallel computing {.allowframebreaks}
...
```
  ...
  The first three lines above load the foreach, iterators, and doFuture packages, the latter being a "backend" for the foreach package.
  The next line tells foreach that we will use the doFuture backend.
  The last line says that we plan to parallelize using multiple `R` sessions on our single machine.
  By default, `R` will attempt to determine how many cores are available and will run an appropriate number of concurrent `R` processes.
...
## Particle filtering in pomp {.allowframebreaks}
  ...
  Here, we repeat the construction of the SIR model [we looked at earlier](https://kingaa.github.io/sbied/stochsim/), using the parameters chosen by looking at simulations.
  `R` code to build the model is available [for download](./model.R).
...
## Profile likelihood estimation
  ...
  We will explain this method below and put it into practice in the [next lesson](https://kingaa.github.io/sbied/mif/).
...
## Confidence intervals via the profile likelihood {.allowframebreaks}
  ...
  - 
    ...
    \end{equation*}
  - 
    This is known as a profile likelihood confidence interval. The cutoff $1.92$ is derived using [Wilks' theorem](https://en.wikipedia.org/wiki/Likelihood-ratio_test\#Distribution:_Wilks.27s_theorem), which we will discuss in more detail when we develop likelihood ratio tests.
  - 
...
```{r like-slice1,eval=FALSE,purl=FALSE}
  slice_design(
    center=coef(measSIR),
...
```{r pfilter-grid1a,eval=FALSE,purl=FALSE}
  expand.grid(
...
```{r pfilter-grid1-eval,include=FALSE}
...
## Wilks' theorem and profile likelihood {.allowframebreaks}
  ...
  - 
    ...
    \end{equation*}
  - 
    We can compute the 95\%-ile for a chi-squared distribution with one degree of freedom: `qchisq(0.95,df=1)`$=`r mysignif(qchisq(0.95,df=1),4)`$.
  - 
  ...
  - 
    This is an example of [a general duality between confidence intervals and hypothesis tests](https://www.stat.nus.edu.sg/~wloh/lecture17.pdf).
...
## Akaike's information criterion (AIC) {.allowframebreaks}
  ...
  - 
    ...
    Increasing the number of parameters leads to additional **overfitting** which can decrease predictive skill of the fitted model.
  - 
  ...
  - 
    ...
    One can penalize complexity by incorporating a more severe penalty than the $2D$ term above, such as via [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion).
  - 

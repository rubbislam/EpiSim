...
-  This tutorial follows on from the [topic of particle filtering](../pfilter/slides.pdf)
  (also known as sequential Monte Carlo) via `pfilter` in pomp.
...
We'll use these results to constrain the infectious period in our model to 3.5~da, i.e., $\gamma=2~\mathrm{wk}^{-1}$.
We fix $k=10$.
...
We proceed to estimate $\beta$, $\eta$, $\sigma$, and $\rho$.


## Sanity check
In Lesson 3, we have introduced how to test the codes and the particle filter from three aspects.
Now we can compare the simulations with the raw data using the proposed parameters:
...
[Lesson 3](https://kingaa.github.io/sbied/pfilter/) discusses how to accomplish this using foreach.
...
We proceed to carry out replicated particle filters at an initial guess of $\beta=`r coef(measSEIR,"Beta")`$, $\eta=`r coef(measSEIR,"Eta")`$, and $\rho=`r coef(measSEIR,"Rho")`$.
...
We already had the likelihood at a single point.
We can store this point, ogether with the estimated likelihood and our estimate of the standard error on that likelihood, in a CSV file:
...
-  Since $\beta$, $\sigma$ and $\gamma$ will be estimated on the log scale, and we expect that multiplicative perturbations of these parameters will have roughly similar effects on the likelihood, we'll use a perturbation size of $0.02$, which we imagine will have a small but non-negligible effect.
...
```{r local_search_plot, eval=FALSE}
mifs_local |>
  traces() |>
  melt() |>
...
$\beta\in (5,80)$, $\sigma\in (1/3,3)$, $\rho\in (0.05,0.95)$, and $\eta\in (0,1)$.
...
-  The above codes run one search from each of `r nrow(results_global1)` starting values.
-  Each search consists of an initial run of `r nrow(traces(mf1))-1` IF2 iterations, followed by another 150 iterations.
...
-  The best result of this search had a likelihood of `r round(max(results_global1$loglik),1)` with a standard error of `r round(results_global1$loglik.se[which.max(results_global1$loglik)],2)`.
...
```{r eta_profile_pairs,eval=FALSE}
read_csv("measles_params.csv") |>
...
```{r eta_profile_plot1,eval=FALSE}
results_eta |>
...
```{r eta_profile_plot2, eval=FALSE}
results_eta |>
  filter(is.finite(loglik)) |>
...
```{r eta_profile_plot3,eval=FALSE}
...
```{r eta_profile_eta_by_rho, eval=FALSE}
results_eta |>
  filter(is.finite(loglik)) |>
  group_by(round(Eta,5)) |>
  filter(rank(-loglik)<3) |>
...
While the above profile trace is suggestive that the 95\% CI for $\rho$ must be between roughly `r signif(100*rho_ci$min,1)`\% and `r signif(100*rho_ci$max,1)`\%, to confirm this, we should construct a proper profile likelihood over $\rho$.
We do so now.
...
-  When we do this for Consett, we see that the reporting efficiency is roughly 20\%.
...
We will estimate the model under the assumption that $\rho=0.2$, but without making assumptions about the duration of the infectious period.
...
To make inferences about $\gamma$, we can again compute a profile likelihood.
...
-  This suggests that $\Rho=0.2$ is consistent only with smaller values of $\gamma$, and hence *longer* infectious periods than are possible if the duration of shedding is actually less than one week.
...
<!-- 1.  {#it:zero} First, conduct a local search and compute the likelihood at the end of each `mif2` run, as shown above. -->
...
<!-- 2.  At run-level 1, we want a quick calculation that verifies that the codes are working as expected. -->
...
<!-- 3.  At run-level 2, we want a computation that gives us some results we can begin to interpret, but that is still as quick as possible. -->
...
<!-- 4.  Run-level 3 is intended for final or near-final results. -->
...
<!-- 5.  How does the SEIR model compare with the SIR model? -->
...
<!-- ##  Fitting all parameters of the SEIR model -->
...
<!-- Check the source code for the `measSEIR` pomp object, using the `spy` command. -->
...
Have a look at [our advice on tuning IF2](./if2_settings.html).

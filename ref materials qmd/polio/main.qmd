...
## Objectives {.allowframebreaks}
  ...
  1.  Demonstrate the use of covariates in pomp to add demographic data (birth rates and total population) and  seasonality to an epidemiological model.
...
## Reviewing covariates in time series analysis {.allowframebreaks}
...
## Covariates in the pomp package {.allowframebreaks}  

  -  pomp provides facilities for including covariates in a pomp object.

  -  Named covariate time series entered via the `covar` argument to `pomp` are automatically defined within C snippets used for the `rprocess`, `dprocess`, `rmeasure`, `dmeasure` and `rinit` arguments.
...
## Polio in Wisconsin {.allowframebreaks}
...
```{r data_plot,echo=FALSE,out.width="0.8\\linewidth",purl=FALSE}
  data |>
...
## {.allowframebreaks}
  ...
  -  The flows through the compartments are graphically represented on the following slide (Figure 1A of @Martinez-Bakker2015):
...
## {.allowframebreaks}
  ...
  \includegraphics[width=6cm]{polio_fig1A} \hspace{-5mm}\parbox[b]{5.5cm}{
...
## {.allowframebreaks}
  ...
  - @Martinez-Bakker2015 fitted monthly reported cases, May 1932 through January 1953, so we set $t_n=1932+ (4+n)/12$ and
...
## {.allowframebreaks}
  ...
  -  We employ a continuous population model, with no demographic stochasticity. Writing $B_n$ for births in month $n$, we obtain the dynamic model of @Martinez-Bakker2015:
    $$\begin{array}{rcl}
    S^B_{1,n+1}&=&B_{n+1}\\
...
## The measurement model {.allowframebreaks}
  - 
...
## Initial conditions {.allowframebreaks}
...
# A \texttt{pomp} representation of the POMP model 

## Building a `pomp` object for the polio model {.allowframebreaks}
...
## Setting up the covariate table {.allowframebreaks}  
  - 
  ...
  - 
    `B` is monthly births.
  -  `xi1,...,xi6`  is a periodic B-spline basis
...
## Regular parameters and initial value parameters {.allowframebreaks}
...
## Fixed parameters (FPs) {.allowframebreaks}
...
## A starting value for the parameters {.allowframebreaks}
...
```{r run_level_set,eval=TRUE,purl=FALSE}
  run_level <- 3
```

```{r run_level_actual,eval=FALSE,purl=TRUE,include=FALSE}
...
## Comments on setting algorithmic parameters {.allowframebreaks}
  

  -  Using `run_level` settings is convenient for editing source code. It plays no fundamental role in the final results. If you are not editing the source code, or using the code as a template for developing your own analysis, it has no function.
  ...
  -  We intend `run_level=1` to run in minutes, `run_level=2` to run in tens of minutes, and `run_level=3` to run in hours.
...
##  Choosing algorithmic parameters
  ...
  [Worked solution to the Exercise](algorithmic-parameters-exercise.html)



## Setting up parallelization {.allowframebreaks}


  We will use doFuture to parallelize our computations.
  At run-levels 1 and 2, it will be sufficient to use the multiple cores on a single machine, but at run-level 3, we will use an HPC cluster.
  In the following, we use our own HPC cluster.
  To run these codes on your own HPC cluster, you would need to edit the \texttt{CLUSTER.R} script suitably.
...
## Likelihood evaluation at the starting parameter estimate {.allowframebreaks}
...
```
  ...
  -  In  `r myround(.system.time[3],1)` seconds (with `r cores` cores) we obtain a log-likelihood estimate of `r myround(L1[1],2)` with a Monte Carlo standard error of `r myround(L1[2],2)`.

  -  Here, we use `stew()` to *cache* the results of the computation.
...
## Caching computations in Rmarkdown {.allowframebreaks}
...
## {.allowframebreaks}
  ...
  -   You may already be familiar the versions of caching provided in {.Rmd} and {.Rnw} files. The argument `cache=TRUE` can be set individually for each chunk or as a global option.

  -   When `cache=TRUE`, Rmarkdown/knitr caches the results of the chunk, meaning that a chunk will only be re-run if code in that chunk is edited.

  -   You can force Rmarkdown/knitr to recompute all the chunks by deleting the `cache` subdirectory.
  




## Practical advice for caching {.allowframebreaks}
  -   What if changes elsewhere in the document affect the proper evaluation of your chunk, but you didn't edit any of the code in the chunk itself?
    Rmarkdown/knitr will get this wrong. *It will not recompute the chunk*.
...
## `stew` and `bake`
  ...
  -   Here, `stew` looks for a file called `pf1.rda`.
  ...
  -   either way, the computation will not be re-run unless you edit the code, change something on which the computation depends, or manually delete the archive file (`pf1.rda`).
...
## Simulation to investigate local persistence {.allowframebreaks}
  - 
  ...
  -  Following @Martinez-Bakker2015, we are interested in how often months with no reported cases ($Y_n=0$) correspond to months without any local asymptomatic cases, defined for our continuous state model as $I^B_n+I^O_n<1/2$.
...
```{r persistence-analysis,include=FALSE}
  sims |>
    group_by(.id) |>
    summarize(
...
```{r persistence_analysis2,include=FALSE}
  summ |>
    column_to_rownames("type") -> summ
polio1 <- polio
```

  For the data, there were `r summ["data","no_cases"]` months with no reported cases, similar to the mean of `r mysignif(summ["sim","no_cases"],3)` for simulations from the fitted model.

  Months with no asymptomatic infections for the simulations were rare, on average `r mysignif(summ["sim","fadeout1"],2)` months per simulation.
  
  Months with fewer than 100 infections averaged `r mysignif(summ["sim","fadeout100"],2)` per simulation, which in the context of a reporting rate of `r mysignif(coef(polio1,"rho"),3)` can explain the absences of case reports.
  
  The mean monthly infections due to importations, modeled by $\psi$, is `r mysignif(summ["sim","imports"],2)`.
...
## Local likelihood maximization {.allowframebreaks}
...
```
  ...
  -  This investigation took `r myround(.system.time[3]/60,1)`~min.
...
```{r pairs,purl=FALSE}
  pairs(~logLik+psi+rho+tau+sigma_dem+sigma_env,
    data=subset(r2,logLik>max(logLik)-20),pch=16)
```
...
## Global likelihood maximization {.allowframebreaks}
...
```
  ...
  -  Evaluation of the best result of this search gives a likelihood of `r myround(max(r3$logLik),1)` with a standard error of `r myround(r3$logLik_se[which.max(r3$logLik)],1)`.
...
```{r pairs_global,purl=FALSE}
  pairs(~logLik+psi+rho+tau+sigma_dem+sigma_env,
    data=subset(r3,logLik>max(logLik)-20),pch=16)
```




## Benchmark likelihoods for non-mechanistic models {.allowframebreaks}
...
```
  ...
  -  A model with likelihood below `r myround(-nb_mle$value,1)` is unreasonable. This explains a cutoff around this value in the global searches: in these cases, the model is finding essentially IID explanations for the data.
  




## ARMA models as benchmarks {.allowframebreaks}
...
```
  ...
  -  This 7-parameter model, which knows nothing of susceptible depletion, attains a likelihood of `r myround(arma_fit$loglik-sum(log_y),1)`.
...
## Mining previous investigations of the likelihood {.allowframebreaks}


```{r param_file,out.width="0.8\\linewidth"}
...
```{r global_rho_code,eval=FALSE,echo=TRUE,purl=FALSE}
  plot(logLik~rho,data=subset(r3,logLik>max(r3$logLik)-10),log="x")
```

  \vspace{-2mm}

```{r global_rho_plot,eval=TRUE,echo=FALSE,out.width="0.6\\linewidth",fig.width=5,fig.height=3,purl=FALSE}
...
## Profile likelihood {.allowframebreaks}
...
```
  ...
  -  It took $`r round(.system.time["elapsed"]/60,1)`$ minutes to run on $`r cores`$ cores.
...
```{r profile_rho_code,echo=T,eval=F}
  <<profile_rho>>
```
...
```{r profile_rho_plot1,echo=FALSE,fig.width=6,fig.height=6,out.width="0.7\\linewidth",purl=FALSE}
  pairs(~logLik+psi+rho+tau+sigma_dem+sigma_env,data=subset(m4,logLik>max(logLik)-10),pch=16)
```

```{r save_profile_rho,echo=FALSE}
  read_csv("params.csv") |>
    bind_rows(m4) |>
    arrange(-logLik) |>
...
##  Diagnosing filtering and maximization convergence {.allowframebreaks}
  ...
  In particular, `cooling.fraction.50` gives the fraction by which the random walk standard deviation is decreased ("cooled") in 50 iterations.
  If `cooling.fraction.50` is too small, the search will "freeze" too soon, evidenced by flat parallel lines in the convergence diagnostics.
  If `cooling.fraction.50` is too large, the researcher may run of of time, patience or computing budget (or all three) before the parameter trajectories approach an MLE.
...
```{r mif_diagnostics1,echo=TRUE,eval=TRUE,purl=FALSE}
...
```{r mif_diagnostics2,echo=TRUE,eval=TRUE,purl=FALSE}
  best |> traces() |> melt() |>
...
```{r likelihood_convergence,out.width="0.8\\linewidth",purl=FALSE}
...
## Acknowledgments and License {.allowframebreaks}
  ...
  -  The materials build on [previous versions of this course and related courses](https://kingaa.github.io/sbied/acknowledge.html).

  -  Produced with R version `r getRversion()` and pomp version `r packageVersion("pomp")`.

  - 
    Licensed under the [Creative Commons attribution-noncommercial license](https://creativecommons.org/licenses/by-nc/3.0/).
